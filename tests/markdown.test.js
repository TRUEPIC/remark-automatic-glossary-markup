...
describe("remark-automatic-glossary-markup", () => {
  ...
  test("Wraps a multiple terms located in the same paragraph", () => {
    ...
    expect(resultString.contents).toEqual("The [Lens SDK](#glossary-Lens%20SDK) is an easy to use SDK and uses [JSON](#glossary-JSON)\n");
  });
  ...
  test("Only wraps the first of a specific term that occurs in a single line.", () => {
    ...
    const resultString = processor.processSync("The Lens SDK is an easy to use SDK.  It is the only Lens SDK you will ever need.\n")
    expect(resultString.contents).toEqual("The [Lens SDK](#glossary-Lens%20SDK) is an easy to use SDK.  It is the only Lens SDK you will ever need.\n");
  });
